...
public class AddNewBranch extends BasePage {
    ...
    public void addBranchWithValidData(String branchName, String Address, String postOfficeBox
        ...
        driver.element().type(OperatingCountry, "Egy");
        new ElementActions().keyPress(OperatingCountry, Keys.ARROW_DOWN).keyPress(OperatingCountry, Keys.ARROW_DOWN).keyPress(OperatingCountry, Keys.ENTER);
                driver.element().type(BranchName, branchName)
                .select(State, "giza") //list
                .select(City, "giza") //list
